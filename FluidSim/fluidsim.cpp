...
FluidSim::FluidSim() : CubeApplication(40) {
}
...
FluidSim::Drop::Drop(Vector3i maxPos, Vector3f pos, Vector3f vel, Vector3f accel, Color col)
...
bool FluidSim::Drop::getRdyDelete(){
    return rdyDelete_;
}

FluidSim::SurfaceParticle::SurfaceParticle(Vector3i maxPos, Vector3f pos, Vector3f vel, Vector3f accel, Color col) :
        Particle(pos, vel, accel, col)
        {
    maxPosition = maxPos;
}

void FluidSim::SurfaceParticle::step(){
    accelerateOnSurface();
    move();
    warp();
}

void FluidSim::SurfaceParticle::accelerateOnSurface(){
...
void FluidSim::SurfaceParticle::warp(){
...
FluidSim::Particle::Particle(Vector3f pos, Vector3f vel, Vector3f accel, Color col)
        :position_(pos),
         velocity_(vel),
         acceleration_(accel),
         color_(col){}

void FluidSim::Particle::step(){
    accelerate();
    move();
}

void FluidSim::Particle::move(){
    position_ += velocity_;
}

void FluidSim::Particle::accelerate(){
    velocity_ += acceleration_;
}

Vector3f FluidSim::Particle::position(){
    return position_;
}

Vector3f FluidSim::Particle::velocity(){
    return velocity_;
}

Vector3f FluidSim::Particle::acceleration(){
    return acceleration_;
}

Vector3i FluidSim::Particle::iPosition(){
    return Vector3i(round(position()[0]),round(position()[1]),round(position()[2]));
}

Vector3i FluidSim::Particle::iVelocity(){
    return Vector3i(round(velocity()[0]),round(velocity()[1]),round(position()[2]));
}

Vector3i FluidSim::Particle::iAcceleration(){
    return Vector3i(round(acceleration()[0]),round(acceleration()[1]),round(acceleration()[2]));
}

void FluidSim::Particle::position(Vector3f pos){
    position_ = pos;
}
void FluidSim::Particle::velocity(Vector3f vel){
    velocity_ = vel;
}
void FluidSim::Particle::acceleration(Vector3f accel){
    acceleration_ = accel;
}

Color FluidSim::Particle::color(){
    return color_;
}
void FluidSim::Particle::color(Color Col){
    color_ = Col;
}
